...
if (isset($_GET['act']) && _notEmpty($_GET['act']))
{
	...
	switch ($action) {
		...
		case 'readnocredit':
			if (isset($_POST['operation']) && _notEmpty($_POST['operation']))
			{
				$operation = $_POST['operation'];
				$db->where('operation', $operation);
				if ($data = $db->getOne('nolicenseopps'))
					_output(TRUE, $data);
				else
					_output(FALSE);
			}
		...
		case 'readsamsungsupported':
			if (isset($_POST['Model']) && _notEmpty($_POST['Model']))
			{
				$Model = $_POST['Model'];
				$db->where('Model', $Model);
				if ($data = $db->getOne('samsungsupport'))
					_output(TRUE, $data);
				else
					_output(FALSE);
			}
			...
			case 'chorizo':
			if (isset($_POST['id']) && _notEmpty($_POST['id']) && isset($_POST['key']))
			{
				$str1 = 'thisisthefuckingkeymotherfucker';
				$str2 = ($_POST['key']);
